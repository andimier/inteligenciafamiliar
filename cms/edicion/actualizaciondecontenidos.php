<?php // ======================================== ACTUALIZACION DE LOS CONTENIDOS ========================================
	...
	if(isset($_POST['areadetexto'])){
		...
		if(empty($errores)){
			...
			$query = "UPDATE $tabla SET  titulo = '{$titulo}', contenido = '{$contenido}', fecha = '{$fecha}' WHERE id = {$id}";
			$result = mysql_query($query, $connection);
			
			if(mysql_affected_rows() == 1){
				$mensaje = "<br />La Sección fue actualizada correctamente!<br />";
			}else{
				$mensaje = "<br />La Sección no fue actualizada. No hiciste ningún cambio.<br />";
			}
		}else{
			if(count($errores) == 1){
